...
The final range-provider check also matched all pixels for real-video frames 150–153 (0.983 seconds for the four-frame decode). The 10-bit source exposed quantization differences in direct 8-bit raw conversion; the provider now preserves the reference PNG conversion. Regression coverage includes a 10-bit fixture, both traversal directions, window boundaries, thumbnail/native separation and mobile navigation. Final local checks passed 29 Python tests, 12 Node tests and the browser suite.
